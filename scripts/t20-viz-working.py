...
# Canonical AI prediction (consensus of Claude, ChatGPT, Perplexity)
canonical_ai = {
    'A1': 'India', 'A2': 'Pakistan',
    'B1': 'Australia', 'B2': 'Sri Lanka',
    'C1': 'England', 'C2': 'West Indies',
    'D1': 'South Africa', 'D2': 'New Zealand'
}
...
plt.tight_layout()
plt.savefig('../../img/t20-2026-ai-similarity.png', dpi=150, bbox_inches='tight')
print("Saved: img/t20-2026-ai-similarity.png")
plt.close()
...
def plot_group_heatmap(group_name):
    ...
    plt.tight_layout()
    plt.savefig(f'../../img/t20-2026-group-{group_name}-heatmap.png', dpi=150, bbox_inches='tight')
    print(f"Saved: img/t20-2026-group-{group_name}-heatmap.png")
    plt.close()
...
for group in ['A', 'B', 'C', 'D']:
    ...
    combos = human_df[cols].apply(lambda x: tuple(sorted([x[0], x[1]])), axis=1)
...
plt.tight_layout()
plt.savefig('../../img/t20-2026-prediction-diversity.png', dpi=150, bbox_inches='tight')
print("Saved: img/t20-2026-prediction-diversity.png")
plt.close()
...
for idx, row in most_contrarian.iterrows():
    print(f"  - {row['Name']} ({row['Location']}): {row['AI_Similarity']:.3f}")
...
for group in ['A', 'B', 'C', 'D']:
    for pos in ['1', '2']:
        ...
        if len(singles) > 0:
            print(f"  Group {group} - Position {pos}:")
...
for loc, count in location_counts.items():
    if pd.notna(loc) and loc.strip():
        print(f"  - {loc}: {count} participants")

print("\n" + "="*60)
print("All visualizations saved to img/ directory")
print("="*60)
